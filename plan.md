...
### Mô tả
Xây dựng các endpoint trả về dữ liệu thống kê dưới dạng JSON để phía Frontend nhận và vẽ biểu đồ tương tác. Backend **không vẽ biểu đồ**, chỉ tính toán và cung cấp dữ liệu số liệu.

### Công cụ sử dụng
- **FastAPI** – Viết API endpoint
- **Pandas** – Tính toán thống kê mô tả, tương quan, phân phối
- **NumPy** – Hỗ trợ tính toán ma trận tương quan
...
1. **Tạo Pydantic schema** (`schemas/eda_schema.py`):
   - Khai báo các class response cho từng loại phân tích:
     - `DataOverviewResponse`: Tổng quan dữ liệu (số dòng, số cột, missing values, duplicates).
     - `TargetDistributionResponse`: Phân phối biến Churn (số lượng và tỷ lệ % của Yes/No).
     - `CorrelationResponse`: Ma trận tương quan giữa các biến số.
     - `FeatureDistributionResponse`: Phân phối giá trị của 1 biến cụ thể (chia theo nhóm Churn).
     - `ChurnRateResponse`: Tỷ lệ churn theo từng giá trị của 1 biến phân loại.

2. **Viết service xử lý logic** (`services/eda_service.py`):
   - Viết hàm `load_train_data()` để nạp file `train.csv` và cache lại (chỉ đọc file 1 lần).
   - Viết các hàm tính toán:
     - `get_data_overview()` → Trả về thống kê tổng quan.
     - `get_target_distribution()` → Trả về phân phối Churn.
     - `get_correlation_matrix()` → Trả về ma trận tương quan (giữa các biến số).
     - `get_feature_distribution(feature_name)` → Trả về phân phối giá trị của 1 biến, chia theo Churn.
     - `get_churn_rate_by_feature(feature_name)` → Trả về tỷ lệ churn của từng nhóm trong 1 biến phân loại.

3. **Viết route** (`routes/eda_route.py`):
   - `GET /api/eda/overview` → Gọi `get_data_overview()`.
   - `GET /api/eda/target` → Gọi `get_target_distribution()`.
   - `GET /api/eda/correlation` → Gọi `get_correlation_matrix()`.
   - `GET /api/eda/distribution/{feature_name}` → Gọi `get_feature_distribution()`.
   - `GET /api/eda/churn-rate/{feature_name}` → Gọi `get_churn_rate_by_feature()`.

4. **Viết unit test** (`tests/test_eda.py`):
   - Test case 1: Gọi `GET /api/eda/overview` → Kỳ vọng mã 200 và response chứa trường `total_rows`, `total_columns`.
   - Test case 2: Gọi `GET /api/eda/target` → Kỳ vọng tổng tỷ lệ % bằng 100.
   - Test case 3: Gọi `GET /api/eda/correlation` → Kỳ vọng ma trận có kích thước đúng (N×N).
   - Test case 4: Gọi `GET /api/eda/distribution/tenure` → Kỳ vọng mã 200 và response chứa dữ liệu histogram.
   - Test case 5: Gọi `GET /api/eda/distribution/INVALID_COLUMN` → Kỳ vọng mã 404.

### Sản phẩm đầu ra
| File | Mô tả |
|------|-------|
| `schemas/eda_schema.py` | Pydantic models cho response EDA |
| `services/eda_service.py` | Logic tính toán thống kê |
| `routes/eda_route.py` | 5 endpoint GET `/api/eda/...` |
| `tests/test_eda.py` | Bộ test tự động (≥ 5 test cases) |

### Tiêu chí nghiệm thu
- [ ] Tất cả 5 endpoint trả về mã 200 khi truy cập hợp lệ.
- [ ] Dữ liệu JSON trả về có cấu trúc rõ ràng, đủ thông tin để Frontend vẽ biểu đồ mà không cần xử lý thêm.
- [ ] Endpoint `/api/eda/distribution/{feature_name}` trả lỗi 404 khi tên biến không tồn tại.
- [ ] Thời gian phản hồi mỗi endpoint dưới **2 giây** (dữ liệu train.csv ~ 600k dòng).
